"""Single-site importer for Allrecipes.com recipes."""
...
_RE_RECIPE_PATH = re.compile(r"^/recipe/\d+/.+")
...
def _fetch_sitemap_xml(session: requests.Session, url: str, *, verbose: bool = False):
    try:
        ...
        if verbose:
            print(f"[allrecipes_com] sitemap fetch failed: {url}: {e}")
        return None
    content = res.content
    if url.endswith(".gz"):
        try:
            ...
            if verbose:
                print(f"[allrecipes_com] sitemap gzip failed: {url}: {e}")
            return None
    try:
        return ElementTree.fromstring(content)
    except Exception as e:
        if verbose:
            print(f"[allrecipes_com] sitemap parse failed: {url}: {e}")
        return None
...
def _fetch_sitemap_slugs(session: requests.Session, *, cap: int, verbose: bool = False) -> list[str]:
    ...
    while to_visit and len(visited) < max_sitemaps and len(out) < cap:
        ...
        for loc in root.findall(".//{*}loc"):
            ...
            if not path.startswith(prefix) or not _RE_RECIPE_PATH.match(path):
                continue
...
def fetch_recipe(session: requests.Session, slug: str) -> ParsedRecipe:
    if slug.startswith("http://") or slug.startswith("https://"):
        ...
        page_url = f"{BASE}/recipe/{slug.strip('/')}/"
